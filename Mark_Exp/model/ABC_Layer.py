...
class ABC_2D_Large(nn.Module):
    ...
    def img_reconstruction(self, hashtable, img):
        ...
        if self.pixel_number*B<= len(hashtable):
            ...
            for batch in range(B):
                hash = torch.concat([hash, hashtable + batch*C*H*W])
        final_img = img.take(self.cnn_hash.reshape(-1).long().to(device))
        final_img[hash.reshape(-1).long().to(device)] = 0
        final_img = final_img.reshape(B, H*W, -1).permute(2,0,1).reshape(-1, B*H*W)
        return final_img
