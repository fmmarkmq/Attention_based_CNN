...
class ABC_Data_Loader(object):
    ...
    def train_data_loader(self):
        ...
        elif self.args.name == "lat":
            loader = METRLADatasetLoader()
            dataset = loader.get_dataset(num_timesteps_in=self.args.history_len, num_timesteps_out=self.args.predict_len)
            train, test_dataset = temporal_signal_split(dataset, train_ratio=self.args.given_snapshots/34249)
        return train
    ...
    def predict_data_loader(self):
        if self.args.name == 'mnist':
            ...
            loader = METRLADatasetLoader()
            dataset = loader.get_dataset(num_timesteps_in=self.args.history_len, num_timesteps_out=self.args.predict_len)
            train, predict = temporal_signal_split(dataset, train_ratio=self.args.given_snapshots/34249)
        return predict